...
$cmd = "SELECT * FROM $ps->t_team_aliases";
if ($filter) {
	$f = $ps->db->escape($filter, false);
	$cmd .= " WHERE team_id LIKE '%$f%' OR alias LIKE '%$f%'";
}
$cmd .= " ORDER BY team_id ASC,alias ASC LIMIT $start,$limit";
$aliases = $ps->db->fetch_rows(1, $cmd);
$total = $ps->db->count($ps->t_team_aliases);
$pager = pagination(array(
	'baseurl'	=> psss_url_wrapper(array('limit' => $limit, 'filter' => $filter)),
	'total'		=> $total,
	'start'		=> $start,
	'perpage'	=> $limit, 
	'pergroup'	=> 5,
...
$cms->crumb('Team Aliases', psss_url_wrapper($php_scnm));
