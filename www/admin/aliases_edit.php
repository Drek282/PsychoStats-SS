...
if ($cancel) {
	previouspage(psss_url_wrapper(array( '_amp' => '&', '_base' => 'aliases.php' )));
}

// load the matching aliases if an ID was given
$alias = array();
if (!empty($id)) {
	$key = 'team_id'; //is_numeric($id) ? 'id' : 'team_id';
	$alias = $ps->db->fetch_row(1, "SELECT * FROM $ps->t_team_aliases WHERE $key=" . $ps->db->escape($id, true) . " LIMIT 1");
	if (!$alias['id']) {
		$data = array('message' => $cms->trans("Unknown team alias specified"));
		$cms->full_page_err(basename(__FILE__, '.php'), $data);
		exit();	
	}
	// load all matching aliases for this team_id
	$list = $ps->db->fetch_list("SELECT alias FROM $ps->t_team_aliases WHERE team_id=" . $ps->db->escape($alias['team_id'], true) . " ORDER BY alias");
	$alias['aliases'] = implode("\n", $list) . "\n";
	$alias['alias_list'] = $list;
}

// delete alias. If $id is numeric then only a single alias is removed, 
// if its a string then all aliases pointing to the team_id will be deleted.
if ($del and $alias) {
	$key = 'team_id'; // is_numeric($id) ? 'id' : 'team_id';
	$ps->db->delete($ps->t_team_aliases, $key, $id);
	previouspage(psss_url_wrapper(array( '_amp' => '&', '_base' => 'aliases.php' )));
}
...
if ($submit) {
	if ($id) {
		// force the team_id if we're editing an alias (read-only)
		$form->input['team_id'] = $alias['team_id'];
	}
	...
	foreach ($list as $a) {
		// don't allow an alias to equal the team_id
		if (strtolower($a) == strtolower($input['team_id'])) {
			$form->error('aliases', $cms->trans("No alias can be the same as the unique ID"));
			$valid = false;
			break;
		}

		// verify the same alias is not assigned to another team_id
		$exists = $ps->db->fetch_list(sprintf("SELECT 1 FROM $ps->t_team_aliases WHERE team_id <> %s AND alias = %s", 
			$ps->db->escape($input['team_id'], true),
			$ps->db->escape($a, true)
		));
		if ($exists) { 
	...
	// if we're adding a new alias make sure the team_id is ... unique
	if (!$id) {
		$exists = $ps->db->fetch_list(sprintf("SELECT 1 FROM $ps->t_team_aliases WHERE team_id = %s LIMIT 1", 
			$ps->db->escape($input['team_id'], true)
		));
		if ($exists) { 
			$form->error('team_id', $cms->trans("This unique ID is already defined"));
			$valid = false;
		}
	}
	...
	if ($valid) {
		...
		if ($id) {
			...
			if ($del) {
				...
				$a = implode(', ', $a_list);
				$uid = $ps->db->escape($alias['team_id'], true);
				$ok = $ps->db->query("DELETE FROM $ps->t_team_aliases WHERE team_id=$uid AND alias IN ($a)");
			}			
		}

		// add new aliases for this team_id
		$new = $id ? array_diff($input['aliases'], $alias['alias_list']) : $input['aliases'];
		if ($ok and $new) {
			foreach ($new as $a) {
				$ok = $ps->db->insert($ps->t_team_aliases, array(
					'id'		=> $ps->db->next_id($ps->t_team_aliases, 'id'),
		...
		if (!$ok) {
			...
			previouspage(psss_url_wrapper('aliases.php'));
		}
/*
...
$cms->crumb('Team Aliases', psss_url_wrapper('aliases.php'));
$cms->crumb('Edit');
