 ...
 *
 *	Version: $Id: divisionnames.php 389 2008-04-18 15:04:10Z lifo $
 */
...
// re-order tags
if ($move and $id) {
	$list = $ps->db->fetch_rows(1, "SELECT id,idx FROM $ps->t_config_divisionnames ORDER BY idx");
	$inc = $move == 'up' ? -15 : 15;
	$idx = 0;
	...
	for ($i=0; $i < count($list); $i++) {
		...
		$ps->db->update($ps->t_config_divisionnames, array( 'idx' => $list[$i]['idx'] ), 'id', $list[$i]['id']);
	}
	unset($submit);
}

$list = $ps->db->fetch_rows(1, "SELECT * FROM $ps->t_team_adv " . $ps->getsortorder($_order));
$total = $ps->get_total_divisions();
...
$cms->crumb('Divisions', psss_url_wrapper(array('_base' => 'divisionnames.php' )));
