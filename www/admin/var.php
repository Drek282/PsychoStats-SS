...
if ($del and $conf['id'] == $id) {
	...
	previouspage(psss_url_wrapper(array( '_amp' => '&', '_base' => 'conf.php', 'ct' => $conf['conftype'], 's' => $conf['section'])));
}
...
*/
	...
	if ($valid) {
		...
		if ($ok) {
			previouspage(psss_url_wrapper(array('_amp' => '&', '_base' => 'conf.php', 'ct' => $input['conftype'], 's' => $input['section'])));
		}
	}
...
$cms->crumb("Config", psss_url_wrapper(array( '_base' => 'conf.php', 'ct' => $ct )));
