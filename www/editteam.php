...
if (!$team_user->userid() or $cms->user->is_admin() or ($team_user->userid() and $ps->conf['main']['allow_username_change'])) {
	$form->field('username');
}
$form->field('password');
...
if ($submit) {
	...
	// this will avoid 0,0 being set when a team saves their profile
	...
	if (!$form->error('username') and $input['username'] != $team_user->username()) {
	...
	if (!$form->error('username') and $input['username'] != '' or ($team_user->userid() and $input['password'] != '')) {
	...
	if ($valid) {
		// setup user record
		$u['username'] = $input['username'] ? $input['username'] : $team_user->username();
