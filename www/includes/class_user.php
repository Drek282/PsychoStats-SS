...
define("PU_ACL_DIVISIONADMIN", 10);
...
// note: this does not de-associate the user from a team profile.
...
// @param $join_team if true will cause the matching team profile to be included in the results.
// @param $filter defines the start,limit,sort,order and search filter.
function get_user_list($join_team = false, $filter = array()) {
	$filter += array(
		'start'       => 0,
		'limit'       => 100,
		'order'       => 'asc',
...
function acl_str($acl) {
	switch ($acl) {
		...
		case PU_ACL_DIVISIONADMIN: 	$str = 'Division Admin'; break;
...
function acl_divisionadmin(){ return PU_ACL_DIVISIONADMIN; }
...
function accesslevels() {
	return array(
		...
		PU_ACL_DIVISIONADMIN	=> $this->acl_str(PU_ACL_DIVISIONADMIN),
