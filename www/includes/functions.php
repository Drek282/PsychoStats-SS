...
function rank_change($args = array()) {
	...
	$dir = "same";
	$diff = sprintf($args['difffmt'], $prevrank - $rank);	# note: LESS is better. Opposite of 'win_percent'.
...
function url($arg = array()) {
	...
	$base = ($arg['_base'] === NULL) ? psss_escape_html($_SERVER['SCRIPT_NAME']) : $arg['_base'];
	$enc = $arg['_encode'] ? 1 : 0;
...
//	while (list($key,$val) = each($ary)) {
		$ary[$key] = psss_escape_html($ary[$key]);
	}
}
		...
		for ($i=1; $i <= $args['pergroup']; $i++) {
			if ($i == $current) {
				...
				$list[] = sprintf("<a href='%s' class='pager-goto'>%d</a>", 
					psss_url_wrapper(array('_base' => $args['baseurl'], $args['startvar'] => ($i-1)*$args['perpage'], '_anchor' => $args['urltail'])), 
					$i
				);
			}
		}
	...
	if (($args['force_prev_next'] and $total) or $current > 1) {
		if ($current > 1) {
			$output = sprintf("<a href='%s' class='pager-prev'>%s</a> ", 
				psss_url_wrapper(array('_base' => $args['baseurl'], $args['startvar'] => ($current-2)*$args['perpage'], '_anchor' => $args['urltail'])), 
				$args['prev']
			) . $output;
		} else {
	...
	if (($args['force_prev_next'] and $total) or $current < $total) {
		if ($current < $total) {
			$output .= sprintf(" <a href='%s' class='pager-next'>%s</a> ", 
				psss_url_wrapper(array('_base' => $args['baseurl'], $args['startvar'] => $current*$args['perpage'], '_anchor' => $args['urltail'])), 
				$args['next']
			);
		} else {
