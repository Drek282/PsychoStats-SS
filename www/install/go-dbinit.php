...
function do_init($games, $mods) {
	...
	// a 'mod' must be selected if 'ss' is selected as a game type.
	// the other games supported at this time do not have mods.
	if (!$mods and in_array('ss',$games)) {
	...
	foreach ($queries as $sql) {
		...
		if ($db->dbtblprefix != 'psss_') {
			$table = preg_replace('/^psss_/', $db->dbtblprefix, $table);
